...
def main():
    ...
    with open(inputFile,'r',encoding='utf-8') as csvfile:
        ...
        for row in reader:
            transBody.append(re.sub(r'\<.*?\>', "", row[0]))
            transBody.append(re.sub(r'\<.*?\>', "", row[1]))
            data.append(row)
    ...
    for i in range(len(data)):
        jaTrans=transResult[i * 2].translatedText
        enTrans=transResult[i * 2 + 1].translatedText
        data[i][0] = data[i][0] + "\n" + jaTrans
        data[i][1] = data[i][1] + "\n" + enTrans 
        data[i].insert(-1,enTrans)
